...
from fin_2 import find_duplicates
...
class DataGenerator:
    def __init__(self, seed, length, repeat_prob, str_len):
        ...
        for _ in range(100):
            self.seed = (self.seed * 54321) % self.prime
    ...
    def _generator(self, only_dups=False):
        ...
        while fw_steps < self.length:
            if next(rng) < self.prime * (self.repeat_prob / (self.repeat_prob + 1)):
                ...
                if only_dups and bw_steps >= self.length:
                    return
...
def test_duplicates(seed, length, repeat_prob, str_len):
    ...
    print(f"Peak memory usage: {peak / 10**6} MB")  # Print peak memory usage
    print(f"Found duplicates: {len(correct)}")
    ...
    assert sorted(results) == sorted(
        correct
    ), "The generates list of duplicates is not correct, got {} and expected {}".format(
        results, correct
    )
...
tests = [
    ...
    ("10M", lambda: test_duplicates(10, 10**7, 0.0001, 340)),
    ("16M", lambda: test_duplicates(10, 16 * 10**6, 0.0001, 340)),
]
